...
| `cache.disable_notification_listener` | Instance-local switch that disables the dedicated `LISTEN auth_changed` connection. Cache version polling remains active and defaults to `10s` unless `cache.poll_interval` is set. Useful for instances connecting through transaction-pooled proxies that cannot preserve PostgreSQL session state. |
| `cache.poll_interval` | Optional instance-local polling interval (for example `10s`). When set, overrides the shared runtime `cache.poll_interval` on this instance. |
...
| `cache.code_store` | Optional instance-local OAuth code/state store, same shape as the stored `cache.code_store` (`active`, `key_prefix`, `redis.address`, `redis.username`, `redis.password`, `redis.client_name`, `redis.cluster`, `redis.tls.*`). Redis Cluster is detected automatically even when `redis.address` contains one bootstrap address; set `redis.cluster: true` to force cluster-aware routing. When `active` is set, this instance ignores the stored `code_store` and uses this one; other instances keep the stored value. The UI shows it as read-only. |
...
| `cache` | `poll_interval`, `code_store` | Fallback version poll interval for the in-memory read model and OAuth2 temporary code/state store. PostgreSQL notifications normally propagate changes immediately; the poll catches changes missed during listener disconnects. `code_store.active` is `database` (default), `memory`, or `redis`. Database and Redis are shared across replicas; memory is single-instance only. `code_store.redis.cluster: true` explicitly selects Redis Cluster; false keeps automatic detection. `code_store.key_prefix` namespaces the Redis keys (`<prefix>code_<id>`); empty keeps the plain `code_<id>` keys. |
...
| `authorize` | `disabled`, `flow_lifetime`, `login_url`, `disable_remember_me` | Local browser-based authorization code flow (`/oauth2/authorize` + consent screen). Pending consents live `10m` by default. `login_url` redirects anonymous browsers to a login page (with `?redirect_path=` back reference, the [`login`](./login) middleware convention); empty shows an error asking to log in first. `disable_remember_me` hides the remember-me checkbox on the consent screen and ignores its form value. |
...
4. The consent form offers a **Remember me** checkbox (hidden with `authorize.disable_remember_me`). Checking it stores the choice in the issued code, so the token exchange mints a remembered session (sliding refresh window up to `refresh_absolute_lifetime`) without the client having to send the `remember_me` token parameter — important for MCP clients like Claude that never send it. `skip_consent` clients get standard sessions.
5. Approval issues a single-use code bound to client + redirect + PKCE + resources and redirects back to `redirect_uri?code=...&state=...`; the client exchanges it at the token endpoint with `code_verifier`.
...
Each key is its own principal: identity claims carry the stable `sub=api-key:<id>`, `preferred_username` as the key name, `principal_type=api_key`, `api_key_id`, optional `owner_user_id` and `email`, plus the key's own `roles` and `permissions` (IDs and names). Email resolves from `details.email`, then the owner's current email. Empty key names fall back to the canonical subject. `POST /auth/v1/check` accepts `api-key:<id>` as alias. Validation endpoint: `POST /auth/oauth2/api-key` with the `X-API-Key` header returns the claims JSON (`401` when the key is unknown, disabled, expired, or its owner is disabled).
...
Session integration: set `api_key: true` on the `session` provider. Session validates `X-API-Key` directly — in-process when the provider uses `auth_middleware`, or over `oauth2.api_key_url` against a remote auth instance — deletes the raw key header, and forwards the claims context downstream. By default `X-User` uses the key email, owner email, key name, then canonical subject fallback; `X-User-Id` uses the key name or canonical subject. IAM authorization continues to use the validated canonical `sub`.
