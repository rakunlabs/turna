  ...
  <div class="grid gap-3 bg-panel p-4">
    <div class="flex flex-wrap items-start justify-between gap-3">
      <div>
        ...
        <h3 class="mt-2 font-display text-3xl leading-none tracking-tight md:text-4xl">Email Code Login</h3>
      </div>
    ...
    {#if magicLink}
      <p class="text-xs leading-4 text-dim">Magic link login is <span class="text-fg">Enabled</span> — manage it on the MAGIC LINK page.</p>
    {/if}
  </div>
  ...
  {:else if section === "template"}
    <div class="grid content-start gap-px bg-line">
      ...
      <label class="grid gap-1 bg-panel p-3">
        ...
        <textarea class="field-t min-h-48 text-xs leading-4" value={bodyTemplate} placeholder="empty = built-in code body" on:input={(event) => setSettingString("email", ["body_template"], inputValue(event))}></textarea>
      </label>
