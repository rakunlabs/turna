	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if m.Operation.Parse.Enable {
				...
				if err != nil {
					return c.JSON(http.StatusMethodNotAllowed, model.MetaData{Message: err.Error()})
				}
				...
				if provider != "" {
					...
					if providerClaimCheck != "" {
						...
						if !ok {
							return c.JSON(http.StatusUnauthorized, model.MetaData{Message: "provider user claim not found"})
						}
					}
				}
				...
				if userClaim == "" {
					...
					if !ok {
						return c.JSON(http.StatusUnauthorized, model.MetaData{Message: "user claim not found"})
					}
				}
				...
				if userID == "" {
					return c.JSON(http.StatusUnauthorized, model.MetaData{Message: "user not found"})
				}
				...
				if !resp.Allowed {
					return c.JSON(http.StatusForbidden, model.MetaData{Message: "forbidden"})
				}
			}
