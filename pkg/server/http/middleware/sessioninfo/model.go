...
func (m *Info) Info(c echo.Context) error {
	...
	if v, err := sessionM.GetStore().Get(c.Request(), sessionM.GetCookieName(c)); !v.IsNew && err == nil {
		...
		// cookie not found
		return c.JSON(http.StatusNotFound, model.MetaData{Message: "cookie not found"})
	}

	// check if token is valid
	token, err := session.ParseToken64(v64)
	if err != nil {
		return c.JSON(http.StatusForbidden, model.MetaData{Message: fmt.Sprintf("cannot parse token: %v", err)})
	}

	// check if token is valid
	claim := claims.Custom{}
	if _, err := sessionM.Action.Token.GetKeyFunc().ParseWithClaims(token.AccessToken, &claim); err != nil {
		return c.JSON(http.StatusForbidden, model.MetaData{Message: err.Error()})
	}
